...
//     let filtrados = data.events.filter(c => categoria.includes(c.category))
//     crearTarjetas(filtrados,'contenedorTarjetasHome')
// }
// function aplicarBusqueda(busqueda){
//     let filtrados = data.events.filter(ev => ev.name.toLowerCase().includes(busqueda.toLowerCase()))
//     crearTarjetas(filtrados,'contenedorTarjetasHome')
// }
...
function aplicarFiltros(eventos, idContenedor){
    ...
    if(busqueda.length>0){
        filtrados = eventos.filter(ev => ev.name.toLowerCase().includes(busqueda.toLowerCase()))
    }
    else{
        filtrados = eventos
    }
